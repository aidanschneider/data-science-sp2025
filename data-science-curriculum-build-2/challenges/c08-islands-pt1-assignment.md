...
- Are there any sources of *real* uncertainty in the percent occupied
  ...
  - The only real uncertainty in this percent should come from the
    number of houses occupied and unoccupied changing
- Are there any sources of *erroneous* uncertainty in the percent
  occupied you calculated?
  - Human error in counting the number of occupied vs unoccupied houses
...
df_q3 <- 
  ...
  group_by(last_name) %>% 
  count() %>% 
  mutate(p = n / sum(nrow(df_sample_seq))) %>% 
  arrange(desc(p))
...
- What last name is most prevalent in `df_sample_seq`?
  - The most prevalent last name is Sorenson (~12% of sample had this
    last name)
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - This sample is not representative of all houses in Helvig as the
    dataset is comprised of the first 25 homes. Had the sample been
    comprised of 25 randomly selected homes, it would have been more
    representative of the whole (a subsample of the dataset).
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - Random sampling allows for a more representative sample of all homes
    in Helvig
  - This is because sequential sampling is vulnerable to biases: the
    first or last n homes in Helvig may have a prevalence of one last
    name, age group, gender, etc. Taking a random sample ensures that we
    don’t accidentally select a homogenous sample, spreading out samples
    to ensure we select a relatively even amount of subjects from
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - The most prevalent last name I found now that I didn’t find in q3 is
    Collins
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - Last name usually indicates that the indivduals are from the same
    family, and thus tend to live in the same household or relatively
    close by.
...
## HINT: Remember that you need to use analysis() when operating on split_df
  ...
  # a confidence `interval using the percentile method
  int_pctl(., estimate)
```
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - We couldn’t make a CI representative of Helvig as the data from
    which the CI is based is not representative of Helvig.
...
- Number of relations: each islander has a list of parents, significant
  others, and children
- Wealth and occupation: each islander has their occupation and total
  wealth noted
- Age: each islander has their age noted
...
- (What population are you going to study?)
  - We are going to take a random sample of the population of Akkeshi.

#### Quantity of interest

- Our QOI is reaction time.

#### Covariates

- Amount of substance ingested versus change in reaction time
- size/age/weight of subject versus reaction to substance dosage,
  quantified by reaction time
...
- This would be an experimental study as we are providing a substance
  (alcohol/cannabis) to the individuals and monitoring their change in
  reaction time. This is not a passive study as we are actively
  introducing an independent variable.

#### Question / Hypothesis

- Does drinking alcohol (or potentially cannabis) influence an
...
- (What steps will you take to collect the data?)
  ...
  - We will first generate a random sample of house numbers based on
    number of houses existant in Akkeshi and number of houses we want to
    survey
  - We will then go to each of these houses and select individuals above
    the age of 21 for ethical considerations
  - Test each individuals reaction time before being provided substance
  - provide substance (alcohol or cannabis)
  - Test each individuals reaction time after being provided substance
  - Record data

- (How will you ensure the data is representative of your chosen
  population?)

- Random sampling to ensure that our sample is a subsample of the total
  (ensures we are drawing equally from the total).
...
- (How will you choose your sample size?)
  ...
  - We will conduct all experiments in one sitting to ensure no other
    factors influence our subjects.
