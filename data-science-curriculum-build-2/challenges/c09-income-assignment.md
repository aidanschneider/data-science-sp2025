...
filename_pop <- "./data/productDownload_2025-03-12T114212/ACSDT5Y2018.B01003-Data.csv"
...
filename_income <- "./data/productDownload_2025-04-03T170247/ACSST5Y2018.S1903-Data.csv"
...
``` r
z_c = qnorm(1-(1-0.99)/2)
df_q4 <- df_q3 %>% 
  mutate(
    income_SE = income_moe/1.645,
    income_lo = income_estimate - z_c * income_SE,
    income_hi = income_estimate + z_c * income_SE,
...
- Document your observations here.
  - The categories with greatest confidence intervals appear to be
    5-person and 6-person family incomes
  - The categories with the smallest confidence intervals appear to be
    the 2-person and 3-person family incomes
- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?
  - It is hard to distinguish between household incomes in Suffolk
    county as they are quite close together on the plot.
- Which counties have the widest confidence intervals?
  - The counties with the widest confidence intervals are Dukes and
    Nantucket
...
df_data %>% 
  ...
  labs(
    title = "Comparison of Population versus Standard Error of Income"
  )
```
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - I notice that the greater the population, the lower the SE, and the
    smaller the population, the higher the SE
  - This trend may exist because of the inverse relationship between
    standard error and sample size. Since populations are effectively
    whole samples, the greater a population the more accurate our data
    about it becomes.
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - Studying smaller counties is more difficult to study as there is
    less data about them and thus less accurate conclusions can be
    drawn.
...
Question: How does the population size of a county affect the difference
in estimated income between family sizes?

``` r
## TODO: Pose and answer your own question about the data

df_q8 <- df_data %>%
  separate(geographic_area_name, into = c("county", "state"), sep = ",") %>% 
  mutate(state = trimws(state)) %>%
  filter(state == "California") 

df_summary <- df_q8 %>% 
  summarise(quantile(population_estimate)) 
```
...
df_q8 <- df_q8 %>% 
  mutate(population_size = case_when(
    population_estimate > 732212 ~ "big",
    population_estimate > 45235 & population_estimate <= 732212 ~ "medium",
    population_estimate > 0 & population_estimate <= 45235 ~ "small"
  ))

df_q8 %>% 
  ggplot(aes(x = category, y = income_estimate, fill = category)) +
  geom_boxplot() +  # Box plot for each family category
  facet_wrap(~population_size) +  # Create separate subplots for each population size
  labs(
    title = "Income Estimates by Family Size and County Population Size",
    x = "Family Size",
    y = "Income Estimate"
  ) +
...
- Counties with larger populations seem to have a higher mean income
  across all family sizes than other county population sizes
- Smaller counties seem to have a smaller range of incomes than all
  other county population sizes
- All the counties seem to share the trait that 4-person family
  households have slightly higher mean income
