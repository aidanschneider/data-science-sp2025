...
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  ...
  - It is an effective predictor as it takes the average temperature across a plane, which is heavily correlated with the other variables we are considering, x and T_norm, as x is position and T_norm is normalized temperature. 
...
- How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
  - These values both indicate how influential a variable is on the model
- Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
  - relying on statistical significance can lead us to leave out values that are physically significant, especially in engineering or scienctific contexts
