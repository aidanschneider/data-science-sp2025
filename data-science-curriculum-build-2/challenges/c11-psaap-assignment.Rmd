...
- Many variables!
- The only variables present in the dataset are about fluid dynamics, radiation, and temperature
- We know that this dataset pertains to air, which makes sense as the values for viscosity and density are small 
...
- Which model is *most accurate*? Which is *least accurate*?
  - The most accurate model is the cheat model, the next most accurate model is the baseline model, and the least accurate model is the non-physical model
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  - Avg_T is in the output category
  - It is an effective predictor as it takes the average temperature across a plane, which is used to calculate T_norm as specified by the equation above 
- Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
  - We would not have access to avg_T as it is an output variable, thus making it an invalid predictor
- What *Category* of variable is `idx`? Does it have any physical meaning?
  - idx is in the Metadata category
  - It doesnt have physical meaning, it just signifies the id of the simulation
...
- Which columns are excluded in the model formula above? What categories do these belong to? Why are these important quantities to leave out of the model?
  - The columns excluded are idx, avg_q, avg_T, and rms_T, which are all either output or metadata categories
  - They were left out because they are outputs, so would not make good predictors 
...
- How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
  - The standard deviations indicate the variation of each of the quantities
  - The regression coefficients for x and T_f shows each of the value's change
  - When combined, these values show the variation of the output based on x and T_f, which then informs us of the relative importance of x and T_f as predictors
- Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
  - relying on statistical significance can lead us to leave out values that are physically significant, especially in engineering or scientific contexts
...
- Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
  ...
  - I can tell this as the actual T_norm values tend to be close to their predicted values (aka more points inside the grey bar)
...
- How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
  - the fit_simple model needs only x as its predictors, while fit_q4 needs all values as its predictors
...
- How much do you trust your model? Why?
  - I moderately trust my model as it uses physically significant predictors that match the design parameters
  - It could be improved by adding more relevant, significant variables
  - the MSE calculations show that the model has relatively good predictive power
- What kind of interval---confidence or prediction---would you use for this task, and why?
  - I would use prediction (which is what I used) because we need to account for uncertainty in the parameters and random variability in future observations
- What fraction of validation cases lie within the intervals you predict? (NB. Make sure to calculate your intervals *based on the validation data*; don't just use one single interval!) How does this compare with `pr_level`?
  - I found that about 93.3% of actual T_norm values fall within the prediction intervals
...
- Are there any other recommendations you would provide?
  ...
  - conduct additional analyses to identify which uncontrolled variables have the strongest impact on T_norm
