...
# available vignettes: colwise, base, grouping, dplyr, programming, rowwise, two-table, in-packages, window-functions
```
...
iris %>% 
  as_tibble() %>% 
  filter(5.1 <= Sepal.Length,
         Sepal.Length <= 6.4)
  
filter(iris, between(Sepal.Length, 5.1, 6.4)) 
```
...
```{r}
?cut_number #package: ggplot2
?parse_number() #package: readr
?row_number() #package: dplyr
```
